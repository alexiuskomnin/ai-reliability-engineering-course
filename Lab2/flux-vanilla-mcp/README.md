...
flux-vanilla-mcp is a Model Context Protocol (MCP) server built with FastMCP featuring dynamic tool loading.
...
2. **Run in Container**:
   ```bash
   docker run -i flux-vanilla-mcp:latest
   ```
...
kubectl get mcpserver flux-vanilla-mcp
```