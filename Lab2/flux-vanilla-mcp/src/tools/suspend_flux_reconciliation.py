"""Suspend Flux reconciliation by setting `spec.suspend: true`.

Works on any reconcilable Flux kind (sources, Kustomization, HelmRelease).
"""
...
TOOL_NAME = "suspend_flux_reconciliation"
...
@mcp.tool(
    ...
    annotations=ToolAnnotations(
        title="Suspend Flux reconciliation",
        readOnlyHint=False,
        destructiveHint=False,
        idempotentHint=True,
    ),
)
def suspend_flux_reconciliation(kind: str, name: str, namespace: str) -> str:
    """Pause Flux reconciliation for a resource by setting spec.suspend=true.
    ...
    with audit_scope(TOOL_NAME, kind=kind, name=name, namespace=namespace) as audit:
        ...
        try:
            ...
            patch_suspend(api_version, kind, name, namespace, suspend=True)
