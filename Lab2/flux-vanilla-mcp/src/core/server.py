...
class DynamicMCPServer:
    ...
    def __init__(
        ...
        prompts_dir: str | None = "src/prompts",
    ):
        ...
        # Track loaded tools and prompts
    ...
    def load_tools(self) -> None:
        ...
        # Load prompts (separate scan; fail-fast on import errors only).
        self._load_prompts()

    def _load_prompts(self) -> None:
        """Scan prompts_dir and import each module; @mcp.prompt() self-registers."""
        if self.prompts_dir is None or not self.prompts_dir.exists():
            return

        prompt_files = [
            f for f in self.prompts_dir.glob("*.py") if f.name != "__init__.py"
        ]
        if not prompt_files:
            return
        ...
        for prompt_file in prompt_files:
            prompts_before = len(asyncio.run(self.mcp.list_prompts()))
            try:
                if not self._import_tool_module(prompt_file, prompt_file.stem):
                    logging.error(f"Failed to load prompt module: {prompt_file.name}")
                    has_errors = True
                    continue
            except Exception as e:
                logging.error(f"Error loading prompt {prompt_file.name}: {e}")
                has_errors = True
                continue

            prompts_after = len(asyncio.run(self.mcp.list_prompts()))
            if prompts_after > prompts_before:
                self.loaded_prompts.append(prompt_file.stem)
                logging.info(f"Loaded prompt module: {prompt_file.stem}")
            else:
                logging.error(
                    f"Prompt file {prompt_file.stem} did not register any prompts"
                )
                has_errors = True
        ...
        if self.loaded_prompts:
            logging.info(f"💬 Successfully loaded {len(self.loaded_prompts)} prompts")
    ...
    def _register_health_route(self) -> None:
        """Add a GET /healthz endpoint for kmcp / kagent readiness probes.

        Cheap by design: confirms the process is alive and the FastMCP
        instance has loaded at least one tool. Does NOT attempt to reach
        the Kubernetes API — that would couple liveness to cluster
        connectivity and cause unnecessary pod restarts during transient
        API server hiccups.
        """
            ...
            return JSONResponse(
                {
                    "status": "ok",
                    "server": self.name,
                    "tools": len(self.loaded_tools),
                    "prompts": len(self.loaded_prompts),
                }
            )
